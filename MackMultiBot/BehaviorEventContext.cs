...
namespace MackMultiBot
{
        public class BehaviorEventContext(ILobby lobby)
        {
                ...
                public async Task<T> UsingApiClient<T>(Func<OsuApiClient, Task<ApiResult<T>>> apiCall) where T : class
                {
                        var result = await apiCall(Lobby.Bot.OsuApiClient);
                        if (result.IsFailure) throw new OsuApiException(result.Error.Message);
                        return result.Value;
                }
        }
}
